...
def parse_arguments():
    ...
    parser.add_argument(
        "--test", default="data/test_dataset_without_labels", help="Directory of testing dataset"
    )
    parser.add_argument(
        "--train", default="data/train_dataset_with_activity", help="Directory of training dataset"
    )
    parser.add_argument("--gt", default=False, help="Is converting gt or prediction")
    parser.add_argument(
        "-o", "--outdir", default="data/trackerjrdb19", help="Directory of KITTI output"
    )
...
def copy_images_and_pointclouds(opt):
    ...
    global_config = os.path.join(opt.train, "calibration", "defaults.yaml")
    ...
    for seq_folder, sequence_paths, indir in zip(
        ...
        for i, seq_path in enumerate(sequence_paths):
            ...
            # copy pointcloads
            seq_folders = ''
            if sequence_folders_mot[0] == seq_folder:
                seq_folders = '{:04d}'.format(train_seqs.index(seq_name))
            else:
                seq_folders = '{:04d}'.format(test_seqs.index(seq_name))
            folder_path_pcs = os.path.join(opt.outdir, seq_folder, "velodyne", seq_folders)
...
def convert_3d(opt):
    ...
    for i, fname in enumerate(sorted(files)):
        ...
        save_path = os.path.join(opt.outdir, "label_02",)
        save_name = os.path.join(save_path, "%04d" % i + ".txt")
        os.makedirs(save_path, exist_ok=True)
        # df.to_csv(save_name, index=False, header=False)
        with open(save_name, "w") as f:
